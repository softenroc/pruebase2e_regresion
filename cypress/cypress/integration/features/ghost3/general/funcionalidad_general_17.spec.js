...
describe("Feature: General", () => {
  ...
  it("Como owner/staff ingreso a General para modificar social accounts Twitter URL con simbolos y retorna error", () => {
...
async function when_i_enter_a_twitter_URL(){
    cy.readFile(path_data+'data_funcionalidad_general_17.json').then(data => {   
...
async function and_i_click_on_save_settings_button(){
  ...
  async function then_validate_text_error() {
    cy.log(" Then Validate text main error equals to <Your Username is not a valid Twitter Username>");
    cy.get('p[class="response"]').should(($p) => {
        const message = $p.text();
        expect(message).contain("Your Username is not a valid Twitter Username");
    });
