...
describe('Feature: Tags', () => {
    ...
    it('Cuando creamos un nuevo tag e ingresamos informacion de facebook card correcta, entonces se debe crear el tag', () => {        
        cy.access_valid_ghost3();
        cy.wait(3000);
        cy.navigate_tags_ghost3();
        cy.click_new_tag_ghost3();
        when_i_entry_a_description_with_facebook_card_data_ok();                          
    })
  });


  async function when_i_entry_a_description_with_facebook_card_data_ok(){ 
    cy.readFile(path_data+'data_funcionalidad_tags_con_facebook_card.json').then(data => {  
