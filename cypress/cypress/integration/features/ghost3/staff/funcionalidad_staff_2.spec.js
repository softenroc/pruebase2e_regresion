...
describe("Feature: Opcion Staf", () => {
  ...
  it(" Como owner ingreso a Staff , link owner y cambio la informacion de mi localizacion con texto que excede el limite ", () => {
    ...
    cy.readFile(path_data+'data_funcionalidad_staff_2.json').then(data => {   
      ...
      cy.log(" Then Validate text main error to location equals to <Location is too long>");
      cy.get('p[class="response"]').should(($p) => {
        const message = $p.text();
        expect(message).contain("Location is too long");
    });
    });
