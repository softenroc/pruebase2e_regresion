...
describe("Feature: General", () => {
  ...
  it("Como owner/staff ingreso a General para hacer el sitio privado, cambio la contraseña de 50 caracteres y guarda", () => {
...
async function when_i_enter_a_password(){
    cy.readFile(path_data+'data_funcionalidad_general_18.json').then(data => {   
        var size = data.length;
        var index = Math.floor(Math.random() * size);
        cy.log("size data pool:" + size);
        cy.log("When I enter a password  " + data[index].password);
