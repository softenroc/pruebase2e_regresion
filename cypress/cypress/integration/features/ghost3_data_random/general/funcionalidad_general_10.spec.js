...
describe("Feature: General", () => {
  ...
  it("Como owner/staff ingreso a General para modificar Meta description por 157 caracteres y guarda", () => {
    cy.access_valid_ghost3();
    cy.wait(4000);
    cy. navigate_general_ghost3();
    cy.click_expand_meta_data();
    when_i_enter_a_meta_description();
...
async function when_i_enter_a_meta_description(){
    cy.readFile(path_data+'data_funcionalidad_general_10.json').then(data => {   
        var size = data.length;
        var index = Math.floor(Math.random() * size);
        cy.log("size data pool:" + size);
        cy.log("When I enter a meta description  " + data[index].description);
        cy.get('textarea[id="metaDescription"]').clear({force: true})
        cy.get('textarea[id="metaDescription"]').type(data[index].description,{force: true});
        cy.wait(2000);
