...
describe('Feature: Tags', () => {
    ...
    it('Cuando creamos un nuevo tag e ingresamos informacion de twitter card con twitter title superior a 70 caracteres, entonces el span informativo de la cantidad de caracteres debe ser de color rojo', () => {        
        cy.access_valid_ghost3();
        cy.wait(3000);
        cy.navigate_tags_ghost3();
        cy.click_new_tag_ghost3();
        when_i_entry_a_description_with_meta_data_with_twitter_title_more_that_70_characters();                          
    })
  });


  async function when_i_entry_a_description_with_meta_data_with_twitter_title_more_that_70_characters(){ 
    cy.readFile(path_data+'data_funcionalidad_tags_con_twitter_title_superior_data.json').then(data => {          
        ...
        cy.get("button[class='gh-btn']").then($buttons => {
            ...
            $buttons.get(1).click();                       
            cy.wait(1000);   
            cy.log("And I enter twitter card data info :"+tag);                  
