...
describe('Feature: Tags', () => {
    ...
    it('Cuando creamos un nuevo tag e ingresamos informacion de facebook card con facebook description superior a 156 caracteres, entonces el span informativo de la cantidad de caracteres debe ser de color rojo', () => {        
        cy.access_valid_ghost3();
        cy.wait(3000);
        cy.navigate_tags_ghost3();
        cy.click_new_tag_ghost3();
        when_i_entry_a_description_with_facebook_card_with_facebook_description_more_that_156_characters();                          
    })
  });


  async function when_i_entry_a_description_with_facebook_card_with_facebook_description_more_that_156_characters(){ 
    cy.readFile(path_data+'data_funcionalidad_tags_con_facebook_description_superior_data.json').then(data => {          
        cy.wait(3000);
        ...
        cy.get("button[class='gh-btn']").then($buttons => {
            ...
            cy.textarea_facebook_description_tags_with(tag.facebook_description);
